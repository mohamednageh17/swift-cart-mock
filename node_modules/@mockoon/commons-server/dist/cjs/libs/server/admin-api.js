...
exports.createAdminEndpoint = void 0;
 ...
 */
const createAdminEndpoint = (app, serverInstance, { statePurgeCallback, getGlobalVariables, setGlobalVariables, purgeGlobalVariables, getDataBucket, getDataBuckets, purgeDataBuckets, getLogs, purgeLogs, envVarsPrefix, updateEnvironment }) => {
    const replayableEvents = [];
    const adminApiPrefix = '/mockoon-admin';
    const events = new sse_1.Sse({
        getInitialEvents: (request) => {
            const maxLogs = typeof request.query.maxlogs === 'string'
                ? parseInt(request.query.maxlogs, 10)
                : undefined;
            const logs = getLogs();
            const limitedLogs = maxLogs != null && !isNaN(maxLogs) && maxLogs > 0
    ...
    app.get(adminApiPrefix, (req, res) => {
        res.send({
            response: "Welcome to Mockoon's admin API. Check the documentation at https://mockoon.com/docs/latest/admin-api/overview/ for more information."
        });
    });
    ...
    app.put(`${adminApiPrefix}/environment`, (req, res) => {
        try {
            const environment = commons_1.EnvironmentSchema.validate(req.body).value;
            if (!environment) {
...
exports.createAdminEndpoint = createAdminEndpoint;
