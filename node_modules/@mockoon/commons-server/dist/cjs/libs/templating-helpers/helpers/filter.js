    ...
    const validate = (payload, condition) => {
        if (condition !== null &&
            ...
            if (typeof payload === 'object') {
                ...
                return keys.every((k) => validate(payload[k], (0, utils_1.fromSafeString)(condition[k])));
            }
            return false;
        }
        return payload === condition;
    };
    const and = (item, conditions, { or }) => conditions.every((subConditions) => {
        if (Array.isArray(subConditions)) {
            return or(item, subConditions);
        }
        return validate(item, subConditions);
    });
    const or = (item, conditions) => conditions.some((subConditions) => {
        if (Array.isArray(subConditions)) {
