...
import { BINARY_BODY, BodyTypes, CORSHeaders, FileExtensionsWithTemplating, GetContentType, GetRouteResponseContentType, IsValidURL, MimeTypesWithTemplating, ParsedJSONBodyMimeTypes, ParsedXMLBodyMimeTypes, RouteType, ServerErrorCodes, StreamingMode, crudRoutesBuilder, dedupSlashes, defaultEnvironmentVariablesPrefix, defaultMaxTransactionLogs, generateUUID, getLatency, routesFromFolder, stringIncludesArrayItems } from '@mockoon/commons';
...
import { fromExpressRequest, fromWsRequest } from '../requests';
...
import { CreateCallbackInvocation, CreateInFlightRequest, CreateTransaction, isBodySupportingMethod, isValidStatusCode, mimeTypeLookup, preparePath, resolvePathFromEnvironment } from '../utils';
import { createAdminEndpoint } from './admin-api';
import { databucketActions } from './crud';
import { BroadcastContext, getSafeStreamingInterval, isWebSocketOpen, messageToString, serveFileContentInWs } from './ws';
/**
...
export class MockoonServer extends EventEmitter {
    constructor(environment, options = {}) {
        ...
        this.delayResponse = (request, response, next) => {
            setTimeout(next, getLatency(this.environment.latency, this.options.enableRandomLatency));
        };
        /**
         * ### Middleware ###
        ...
        this.options = Object.assign(Object.assign(Object.assign({}, this.options), options), { envVarsPrefix: (_a = options.envVarsPrefix) !== null && _a !== void 0 ? _a : defaultEnvironmentVariablesPrefix });
    }
    /**
     * Start a server
     */
    start() {
        ...
        const webSocketRoutes = routesFromFolder(this.environment.rootChildren, this.environment.folders, this.environment.routes, this.options.disabledRoutes, [RouteType.WS]);
    ...
    setRoutes(server) {
        ...
        const routes = routesFromFolder(this.environment.rootChildren, this.environment.folders, this.environment.routes, this.options.disabledRoutes, [RouteType.HTTP, RouteType.CRUD]);
    ...
    createWebSocketConnectionHandler(webSocketServer, routeFor) {
        return (socket, request) => {
            ...
            const inflightRequest = CreateInFlightRequest(websocketId, request, route);
            this.emit('ws-new-connection', inflightRequest);
            let responseNumber = 1;
            // handle error event
            socket.on('error', (err) => {
                this.emit('error', ServerErrorCodes.WS_SERVING_ERROR, err, baseErrorMeta);
            });
            ...
            socket.on('message', (data, isBinary) => {
                if (isBinary) {
                    this.emit('error', ServerErrorCodes.WS_UNSUPPORTED_CONTENT, null, baseErrorMeta);
                    return;
                }
                const routeInMessage = this.getRefreshedRoute(route);
                // the route is not found. Skip reacting.
                if (!routeInMessage) {
                    this.emit('error', ServerErrorCodes.WS_UNKNOWN_ROUTE, null, baseErrorMeta);
                    return;
                }
                ...
                const enabledRouteResponse = new ResponseRulesInterpreter(routeInMessage.responses, serverRequest, routeInMessage.responseMode, this.environment, this.processedDatabuckets, this.globalVariables, this.options.envVarsPrefix, this.options.publicBaseUrl).chooseResponse(responseNumber, messageData);
                ...
                setTimeout(() => {
                    ...
                    if (content) {
                        socket.send(content || '', (err) => {
                            if (err) {
                                this.emit('error', ServerErrorCodes.WS_SERVING_ERROR, err, Object.assign(Object.assign({}, baseErrorMeta), { selectedResponseUUID: enabledRouteResponse.uuid, selectedResponseLabel: enabledRouteResponse.label }));
                            }
                        });
                    }
    ...
    deriveFinalResponseContentForWebSockets(socket, route, enabledRouteResponse, request, data, connectedRequest) {
        ...
        finalRequest !== null && finalRequest !== void 0 ? finalRequest : (finalRequest = request ? fromWsRequest(request, route, data) : undefined);
        ...
        else if (enabledRouteResponse.bodyType === BodyTypes.FILE &&
            enabledRouteResponse.filePath) {
            const filePath = this.getSafeFilePath(enabledRouteResponse.filePath, finalRequest);
            serveFileContentInWs(socket, route, enabledRouteResponse, this, filePath, (contentData) => TemplateParser({
    ...
    handleOneToOneStreamingResponses(socket, route, request, baseErrorMeta) {
        ...
        const intervalRef = setInterval(() => {
            const enabledRouteResponse = new ResponseRulesInterpreter(route.responses, fromWsRequest(request, route), route.responseMode, this.environment, this.processedDatabuckets, this.globalVariables, this.options.envVarsPrefix, this.options.publicBaseUrl).chooseResponse(responseNumber);
            if (!enabledRouteResponse) {
                return;
            }
    ...
    serveWsResponse(client, content, errorMetaData) {
        client.send(content, (err) => {
            if (err) {
                this.emit('error', ServerErrorCodes.WS_SERVING_ERROR, err, errorMetaData);
            }
        });
    }
    /**
    ...
    createRouteHandler(route, crudId) {
        return (request, response, next) => {
            ...
            const enabledRouteResponse = new ResponseRulesInterpreter(currentRoute.responses, fromExpressRequest(request), currentRoute.responseMode, this.environment, this.processedDatabuckets, this.globalVariables, this.options.envVarsPrefix, this.options.publicBaseUrl).chooseResponse(this.requestNumbers[route.uuid]);
            if (!enabledRouteResponse) {
                return next();
            }
            ...
            const latency = getLatency(enabledRouteResponse.latency, this.options.enableRandomLatency);
    ...
    executeCallbacks(routeResponse, request, response) {
        ...
        if (callbackChainArray.includes(routeResponse.uuid)) {
            this.emit('error', ServerErrorCodes.CALLBACK_ERROR, new Error('Infinite callback loop detected'), {
        ...
        if (routeResponse.callbacks && routeResponse.callbacks.length > 0) {
            ...
            for (const callbackInvocation of routeResponse.callbacks) {
                ...
                try {
                    ...
                    if (callback.bodyType === BodyTypes.DATABUCKET &&
                    ...
                    else if (callback.bodyType === BodyTypes.FILE &&
                        callback.filePath) {
                        try {
    ...
    sendFile(route, routeResponse, routeContentType, request, response) {
        ...
        try {
            ...
            if (!routeResponse.sendFileAsBody) {
                response.set('Content-Disposition', `attachment; filename="${encodeURIComponent(basename(filePath))}"`);
            }
    ...
    buildTLSOptions(environment) {
        ...
        let tlsOptions = {};
        const processTemplating = (content) => TemplateParser({
            content,
            shouldOmitDataHelper: false,
            environment,
            processedDatabuckets: this.processedDatabuckets,
        ...
        if (((_a = environment.tlsOptions) === null || _a === void 0 ? void 0 : _a.pfxPath) ||
            ...
            if (((_d = environment.tlsOptions) === null || _d === void 0 ? void 0 : _d.type) === 'PFX' &&
                ...
                tlsOptions.pfx = readFileSync(resolvePathFromEnvironment(processTemplating((_f = environment.tlsOptions) === null || _f === void 0 ? void 0 : _f.pfxPath), this.options.environmentDirectory));
            }
                ...
                tlsOptions.cert = readFileSync(resolvePathFromEnvironment(processTemplating((_k = environment.tlsOptions) === null || _k === void 0 ? void 0 : _k.certPath), this.options.environmentDirectory));
                tlsOptions.key = readFileSync(resolvePathFromEnvironment(processTemplating((_l = environment.tlsOptions) === null || _l === void 0 ? void 0 : _l.keyPath), this.options.environmentDirectory));
            }
            if ((_m = environment.tlsOptions) === null || _m === void 0 ? void 0 : _m.caPath) {
                tlsOptions.ca = readFileSync(resolvePathFromEnvironment(processTemplating((_o = environment.tlsOptions) === null || _o === void 0 ? void 0 : _o.caPath), this.options.environmentDirectory));
            }
    ...
    generateDatabuckets(environment) {
        if (environment.data.length > 0) {
            environment.data.forEach((databucket) => {
                ...
                if (new RegExp(`{{2,3}[#(~\\s\\w ]*((?<![\\w])${requestHelperNames.join('|')})[)} ~]+`).exec(databucket.value)) {
    ...
    emitProcessedDatabuckets() {
        ...
        this.processedDatabuckets.map((_a) => {
            var { value: _value } = _a, props = __rest(_a, ["value"]);
