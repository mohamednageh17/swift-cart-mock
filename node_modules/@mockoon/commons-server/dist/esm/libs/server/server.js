import { BINARY_BODY, BodyTypes, CORSHeaders, defaultEnvironmentVariablesPrefix, defaultMaxTransactionLogs, FileExtensionsWithTemplating, GetContentType, getLatency, GetRouteResponseContentType, IsValidURL, MimeTypesWithTemplating, ParsedJSONBodyMimeTypes, ParsedXMLBodyMimeTypes, RouteType, ServerErrorCodes, stringIncludesArrayItems } from '@mockoon/commons';
...
import { CreateCallbackInvocation, CreateTransaction, dedupSlashes, isBodySupportingMethod, preparePath, resolvePathFromEnvironment, routesFromFolder } from '../utils';
...
export class MockoonServer extends EventEmitter {
    ...
    setRoutes(server) {
        ...
        const routes = routesFromFolder(this.environment.rootChildren, this.environment.folders, this.environment.routes, this.options.disabledRoutes);
    ...
    createRouteHandler(route, crudId) {
        return (request, response, next) => {
            ...
            const enabledRouteResponse = new ResponseRulesInterpreter(currentRoute.responses, request, currentRoute.responseMode, this.environment, this.processedDatabuckets, this.globalVariables, this.options.envVarsPrefix).chooseResponse(this.requestNumbers[route.uuid]);
