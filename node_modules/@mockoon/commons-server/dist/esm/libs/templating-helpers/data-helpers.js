...
import { del as objectDel, get as objectGet, push as objectPush, set as objectSet } from 'object-path';
...
export const DataHelpers = function (processedDatabuckets) {
    return {
        data: function (...args) {
            ...
            const targetDatabucket = getDataBucket(processedDatabuckets, fromSafeString(parameters[0]));
            if (targetDatabucket === undefined) {
                return;
            }
            const value = getValueFromPath(targetDatabucket.value, fromSafeString(parameters[1]), '');
        ...
        dataRaw: function (...args) {
            ...
            return getValueFromPath(targetDatabucket.value, fromSafeString(parameters[1]), '');
        },
        /**
        ...
        setData: function (...args) {
            ...
            const targetDatabucket = getDataBucket(processedDatabuckets, fromSafeString(parameters[1]));
            if (targetDatabucket === undefined) {
                return;
            }
            const path = convertPathToArray(String((_a = fromSafeString(parameters[2])) !== null && _a !== void 0 ? _a : ''));
            ...
            if (operator === 'set') {
                ...
                if (typeof currentValue === 'object' &&
                    ...
                    newValue !== null) {
                    if (path) {
                        objectSet(targetDatabucket.value, path, Object.assign(Object.assign({}, currentValue), newValue));
                    }
                    else {
                ...
                if (path && !isNaN(Number(objectGet(targetDatabucket.value, path)))) {
                    objectSet(targetDatabucket.value, path, Number(objectGet(targetDatabucket.value, path)) +
                ...
                if (path &&
                    ...
                    objectSet(targetDatabucket.value, path, !objectGet(targetDatabucket.value, path));
                }
