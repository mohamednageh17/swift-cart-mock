import { ReorderActionType, ReorderableContainers } from '../models/reorder.model';
import { RouteType } from '../models/route.model';
import { findRouteOrFolderContainer, insertItemAtTarget, moveItemAtTarget, sortByUuidsList } from '../utils/mutator-utils';
/**
...
export const reorderRoutesMutator = (environment, reorderAction) => {
    ...
    const containerId = findRouteOrFolderContainer(reorderAction.sourceId, environment);
 ...
 * @returns
 */
export const fullReorderEntitiesMutator = (environment, entity, order, parentId) => {
    switch (entity) {
        case ReorderableContainers.ROUTES:
            if (parentId === 'root') {
                return Object.assign(Object.assign({}, environment), { rootChildren: sortByUuidsList(environment.rootChildren, order) });
            }
            else {
                return Object.assign(Object.assign({}, environment), { folders: environment.folders.map((folder) => {
                        if (folder.uuid === parentId) {
                            return Object.assign(Object.assign({}, folder), { children: sortByUuidsList(folder.children, order) });
                        }
                        return folder;
                    }) });
            }
        case ReorderableContainers.ROUTE_RESPONSES:
            return Object.assign(Object.assign({}, environment), { routes: environment.routes.map((route) => {
                    if (route.uuid === parentId) {
                        return Object.assign(Object.assign({}, route), { responses: sortByUuidsList(route.responses, order) });
                    }
                    return route;
                }) });
        case ReorderableContainers.DATABUCKETS:
            return Object.assign(Object.assign({}, environment), { data: sortByUuidsList(environment.data, order) });
        case ReorderableContainers.CALLBACKS:
            return Object.assign(Object.assign({}, environment), { callbacks: sortByUuidsList(environment.callbacks, order) });
        default:
            return environment;
    }
};
