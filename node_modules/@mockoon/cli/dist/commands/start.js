...
class Start extends core_1.Command {
    ...
    async run() {
        ...
        if (userFlags.data.some((dataFilePath) => dataFilePath.startsWith(config_1.Config.cloudScheme) && !userFlags.token)) {
            this.error('A token is required to load cloud environments. Use the --token flag or set the MOCKOON_CLOUD_TOKEN environment variable.');
        }
        try {
            for (const [index, dataFilePath] of userFlags.data.entries()) {
                const parsedEnvironment = await (0, data_1.parseDataFile)(dataFilePath, {
                    port: userFlags.port[index],
                    hostname: userFlags.hostname[index],
                    proxy: userFlags.proxy
                }, userFlags.repair, userFlags.token);
                const server = this.createServer({
                    ...
                    environmentDirectory: (_a = (0, utils_1.getDirname)((0, path_1.resolve)(parsedEnvironment.originalPath))) !== null && _a !== void 0 ? _a : '',
                ...
                if (!dataFilePath.startsWith('http') && userFlags.watch) {
                    const watcher = (0, chokidar_1.watch)(dataFilePath, {
                        ignoreInitial: true,
                        persistent: false,
                        usePolling: true,
                    ...
                    watcher.on('change', async (changedFilePath) => {
                        try {
                            const parsedNewEnvironment = await (0, data_1.parseDataFile)(changedFilePath, {
                                port: userFlags.port[index],
                                hostname: userFlags.hostname[index],
                                proxy: userFlags.proxy
                            }, userFlags.repair, userFlags.token);
                            ...
                            if (error instanceof Error) {
                                this.error(`Error while processing file change for ${changedFilePath}: ${error.message}`);
                            }
                        }
                    });
                }
...
Start.flags = {
    ...
    ...command_constants_1.logTransactionFlag,
    ...
    'public-base-url': core_1.Flags.string({
        description: 'Public base URL used to resolve relative callback URLs and for the baseUrl templating helper (e.g. https://api.example.com or http://localhost:3000). Must include the protocol and port if non-standard.',
        multiple: true,
        default: []
    }),
    ...
    proxy: core_1.Flags.string({
        description: "Override the environment's proxy settings",
        options: ['enabled', 'disabled']
    }),
    watch: core_1.Flags.boolean({
        char: 'w',
        description: 'Watch local data file(s) for changes and restart the server when a change is detected',
        default: false
    }),
    'polling-interval': core_1.Flags.integer({
